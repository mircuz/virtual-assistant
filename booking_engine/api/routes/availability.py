...
async def check_availability(shop_id: UUID, service_ids: str = Query(..., description="Comma-separated service UUIDs"), start_date: date = Query(...), end_date: date = Query(...), staff_id: UUID | None = Query(None), pool: AsyncConnectionPool = Depends(get_pool)):
    ...
    if not slots and staff_id:
        ...
        suggestions = await get_available_slots(pool, shop_id, parsed_ids, start_date, fallback_end, staff_id=None)
